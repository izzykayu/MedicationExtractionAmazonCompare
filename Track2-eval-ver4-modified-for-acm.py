    ...
    def __init__(self, folder1, folder2, track_num):
        ...
        if not common_files:
            ...
            if files1 - common_files:
                f_missing = open("files-missing.tsv", "w")
                print('Files skipped in {}:'.format(self.folder1))
               # print(sorted(list(files1 - common_files)))
                for mf in sorted(list(files1 - common_files)):
                    new_line_mf = str(mf.split('.')[0]) + "\t" + mf + "\t" + str(mf.split('.')[0]) + '.txt' + os.linesep
                #print(', '.join(sorted(list(files1 - common_files))))
                    print(new_line_mf)
                    f_missing.write(new_line_mf)
            if files2 - common_files:
                print('Files skipped in {}:'.format(self.folder2))
                print(sorted(list(files2 - common_files)))
...
if __name__ == '__main__':
    ...
    main(os.path.abspath(args.folder1), os.path.abspath(args.folder2), 2, True)
    print('\n###################\n')
